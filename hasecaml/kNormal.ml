...
let rec fv = function (* 式に出現する自由な変数 (caml2html: knormal_fv) *)
  ...
  | Tuple(xs) | ExtFunApp(_, xs) | GlobalTuple(xs) -> S.of_list xs
...
let rec g env = function (* K正規化ルーチン本体 (caml2html: knormal_g) *)
  ...
  | Syntax.Mult(e1, e2) -> (* ­足し算のk正規化(caml2html: knormal_add) *)
      insert_let (g env e1)
        (fun x -> insert_let (g env e2)
            (fun y -> Mult(x, y), Type.Int))
  | Syntax.Div(e1, e2) -> (* ­足し算のk正規化(caml2html: knormal_add) *)
      insert_let (g env e1)
        (fun x -> insert_let (g env e2)

...
let fv e = S.diff (fv e) (S.of_list !global_lis)
...
let rec de1 e = match e with
  | [] -> ()
  | x::rest -> print_string x; print_string "\n"; de1 rest

let rec global_check e = match e with
  | KNormal.Let((x, t), GlobalTuple(_), e1) -> global_lis := x::!global_lis; global_check e1
  | KNormal.Let((x, t), GlobalArray(_), e1) -> global_lis := x::!global_lis; global_check e1
  | KNormal.Let((x, t), e1, e2) -> global_check e1; global_check e2
  | KNormal.IfEq(i1, i2, e1, e2) -> global_check e1; global_check e2
  | KNormal.IfLE(i1, i2, e1, e2) -> global_check e1; global_check e2
  | KNormal.LetRec(_, e1) -> global_check e1
  | KNormal.LetTuple (_, _, e1) -> global_check e1
  | _ -> ()
...
let f e =
  ...
  print_string "global化されたものたち\n";
  de1 !global_lis;
  Prog(List.rev !toplevel, e', !global_lis)
